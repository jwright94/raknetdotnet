...
//    December 1, 1995, is provided with ìRestricted RightsÅEas set forth in
...
namespace CommandLine
{
    ...
    public sealed class Parser
    {
                    ...
                    if (argument.Length > 0)
                    {
                        switch (argument[0])
                        {
                            ...
                            case '/':
                                ...
                                string option =
                                    argument.Substring(1, endIndex == -1 ? argument.Length - 1 : endIndex - 1);
        ...
        public string GetUsageString(int screenWidth)
        {
            ...
            foreach (ArgumentHelpStrings helpStrings in strings)
            {
                ...
                while (index < helpStrings.help.Length)
                {
                    ...
                    {
                        endIndex =
                            helpStrings.help.LastIndexOf(' ', endIndex - 1, Math.Min(endIndex - index, charsPerLine));
                        if (endIndex <= index)
                        {
        ...
        private bool LexFileArguments(string fileName, out string[] arguments)
        {
            ...
            {
                this.reporter(
                    string.Format("Error: Can't open command line argument file '{0}' : '{1}'", fileName, e.Message));
                arguments = null;
                return false;
            }
        ...
        private class Argument
        {
            ...
            {
                this.reporter(
                    string.Format("'{0}' is not a valid value for the '{1}' command line option", value, this.LongName));
            }
